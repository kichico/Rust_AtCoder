...
fn solve() {
    ...
    let mut g = ac_library::dsu::Dsu::new(n);
    let mut roots: Vec<BTreeSet<Reverse<usize>>> = vec![BTreeSet::new(); n];
    for i in 0..n {
        roots[i].insert(Reverse(i + 1));
    }
    for (x, mut u, mut v) in query {
        if x == 1 {
            u -= 1;
            v -= 1;
            let root_u = g.leader(u);
            let root_v = g.leader(v);
            g.merge(u, v);
            let new_root = g.leader(u);
            if roots[root_u].len() >= roots[root_v].len() {
                ...
                let vec = std::mem::take(&mut roots[root_u]);
                for elem in vec {
                    roots[new_root].insert(elem);
                }
            }
            let mini = 20.min(roots[new_root].len().clone());
            let vec = std::mem::take(&mut roots[new_root]).into_iter().take(mini);
            roots[new_root] = BTreeSet::from_iter(vec);
        } else {
            u -= 1;
            let root = g.leader(u);
            if v > roots[root].len() {
                ...
                let it = roots[root].iter().take(v).last().unwrap();
                println!("{}", it.0);
            }
        }
    }
