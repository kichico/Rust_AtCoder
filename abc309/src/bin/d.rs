...
fn solve() {
    input! {
        n1:usize,n2:usize,m:usize,edge:[(Usize1,Usize1);m]
    }
    let mut g1 = Dijkstra::new(n1);
    let mut g2 = Dijkstra::new(n2);
    for (u, v) in edge {
        if u < n1 {
            g1.add_edge(u, v, 1);
            g1.add_edge(v, u, 1);
        } else {
            g2.add_edge(u - n1, v - n1, 1);
            g2.add_edge(v - n1, u - n1, 1);
        }
    }
    let mut dist1 = g1.calc_shortest_distance(0);
    let mut dist2 = g2.calc_shortest_distance(n2 - 1);
    dist1.sort();
    dist2.sort();
    let farest1 = dist1.iter().next_back().unwrap();
    let farest2 = dist2.iter().next_back().unwrap();
    println!("{}", farest1 + farest2 + &1);
}
fn main() {
    solve();
}
