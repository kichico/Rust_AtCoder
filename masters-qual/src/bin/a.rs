...
fn solve() {
    ...
    let mut highest = 0;
    let mut rng = rand::thread_rng();
    let mut shokikai = Vec::new();
    let mut best_start_takahashi = (0, 0);
    let mut best_start_aoki = (0, 0);

    let dir_converter = hashmap! {'U'=>0,'D'=>1,'L'=>2,'R'=>3};
    while start.elapsed().as_millis() < 1300 {
        ...
        let mut takahashi_p = Vec::new();
        let mut aoki_p = Vec::new();
        let start_t = takahashi.clone();
        let start_a = aoki.clone();
        for _i in 0..4 * N * N {
            let mut current_step = Vec::new();
            let (ty, tx) = takahashi;
            let (ay, ax) = aoki;
        ...
        if this_step_score > highest {
            ...
            ans = output.clone();
            shokikai = first_result;
            best_start_aoki = start_a;
            best_start_takahashi = start_t;
        }
    }
    let t0 = 2000;
    let t1 = 800;
    let mut t = t0.clone();
    while start.elapsed().as_millis() < 1980 {
        let idx = rng.gen_range(0..4 * N * N);
        let mut another = shokikai.clone();
        let v = &mut another[idx];
        if v[0] == '1' {
            v[0] = '0';
        } else {
            v[0] = '1';
        }
        let takahashi = best_start_takahashi.clone();
        let aoki = best_start_aoki.clone();
