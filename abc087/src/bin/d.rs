...
fn solve() {
    input! {
        n:usize,m:usize,info:[(Usize1,Usize1,i64);m]
    }
    let mut g = PotentializedUnionFind::new(n);
    for (x, y, potential) in info {
        if g.equiv(x, y) && g.get_diff(x, y) != potential {
            //println!("{} {}", potential, g.get_diff(x, y));
            println!("No");
            return;
        }
        g.unite(x, y, potential);
    }
    println!("Yes");
}
fn main() {
