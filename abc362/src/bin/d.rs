...
fn solve() {
    input! {
        n:usize,m:usize,w:[usize;n],edge:[(Usize1,Usize1,usize);m]
    }
    let mut g = Dijkstra::new(n);
    for (from, to, cost) in edge {
        g.add_edge(from, to, cost + w[to]);
        g.add_edge(to, from, cost + w[from]);
    }
    let ans = g.calc_shortest_distance(0);

    println!("{}", (1..n).into_iter().map(|x| ans[x] + w[0]).join(" "));
}
fn main() {
    solve();
}
