...
fn solve() {
    ...
    let bt: BTreeSet<(usize, usize)> =
        BTreeSet::from_iter(a.clone().into_iter().enumerate().map(|x| (x.1, x.0)));
    let total = a.iter().sum::<usize>();
    let mut uf = UnionFind::new(n);

    for i in 0..n {
        let r = bt.range((a[i], 0)..(a[i], n));
        for (_v, idx) in r {
            if uf.equiv(i, *idx) {
                break;
            }
        ...
        let r = bt.range((((a[i] + 1) % m), 0)..(((a[i] + 1) % m), n));
        for (_v, idx) in r {
            if uf.equiv(i, *idx) {
                break;
            }
    ...
    for i in 0..n {
        let e = groups.entry(uf.find_root(i)).or_default();
        e.push(i);
    }
    let mut ans = total.clone();
    for (_root, group) in groups {
        let removal = group.iter().map(|x| a[*x]).sum::<usize>();
        ans = ans.min(total - removal);
    }
    println!("{}", ans);
}
fn main() {
