...
fn solve() {
    input! {
        n:usize,k:usize,mut teeth:Chars
    }
    let mut uf = UnionFind::new(n);
    for i in 0..n - 1 {
        if teeth[i] == 'O' && teeth[i + 1] == 'O' {
            uf.unite(i, i + 1);
        }
    }
    let roots: HashSet<usize> = HashSet::from_iter(
        (0..n)
            .into_iter()
            .map(|x| uf.find_root(x))
            .filter(|f| teeth[*f] != 'X'),
    );
    let mut ans = 0;
    for root in roots {
        //rintln!("{} {}", root, uf.size(root) as usize / k);
        ans += uf.size(root) as usize / k;
    }
    println!("{}", ans);
}
fn main() {
