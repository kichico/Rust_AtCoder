...
fn solve() {
    ...
    let mut sum = 0;
    for (i, (l, r, cost)) in enumerate(query) {
        let mut add_range: Vec<(usize, usize)> = Vec::new();
        let ranges = rangeset.st.range((Included((l, l)), Unbounded));
        let ranges = ranges.into_iter().collect::<Vec<_>>();
        if !ranges.is_empty() {
            let (fl, mut fr) = ranges[0];
            if *fl > l {
                add_range.push((l, *fl));
            }
            for i in 1..ranges.len() {
                add_range.push((fr, ranges[i].0));
                fr = ranges[i].1;
            }
            println!("r:{} fr:{}", r, fr);
            if fr <= r {
                add_range.push((fr, r + 1));
            }
        } else if i != 0 {
            println!("-1");
            return;
        } else {
            add_range.push((l, r + 1));
        }
        for (al, ar) in add_range {
            rangeset.insert(al, ar);
            sum += cost * (ar - al);
        }
        if i != 0 {
            sum += cost;
        }
        for (ll, rr) in &rangeset.st {
            println!("[{},{})", ll, rr);
        }
    }
    println!("{}", sum);
