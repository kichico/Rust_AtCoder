...
fn solve() {
    input! {
        n:usize,m:usize,kukan:[(usize,usize);n]
    }
    let mut ends: MultiSet<usize> = MultiSet::new();
    let total = m * (m + 1) / 2;
    let mut kukanset = BTreeSet::new();
    for (from, to) in &kukan {
        ends.insert(*to);
        kukanset.insert((*from, *to));
    }
    let mut banned = 0;
    for i in 1..=m {
        if let Some((from, to)) = kukanset.range((i, i)..).next() {
            
        }
    }
    println!("{}", total - banned);
}
fn main() {
    solve();
}
