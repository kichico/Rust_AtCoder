...
fn solve() {
    input! {
        n:usize,mut a:[usize;n],b:[usize;n-1]
    }
    let mut bbox: MultiSet<usize> = MultiSet::new();
    for i in 0..n - 1 {
        bbox.insert(b[i]);
    }
    a.sort();
    a.reverse();
    let mut unbox = usize::MAX;
    for i in 0..n {
        if let Some(found) = bbox.check.range(a[i]..).next() {
            bbox.remove(*found);
        } else if unbox == usize::MAX {
            unbox = a[i];
        } else {
            println!("-1");
            return;
