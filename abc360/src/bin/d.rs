...
fn solve() {
    input! {
        n:usize,t:i64,s:Chars,x:[i64;n]
    }
    let mut plus: Vec<i64> = Vec::new();
    ...
    for i in 0..n {
        if s[i] == '1' {
            let l = minus.lower_bound(&x[i]);
            if l == minus.len() {
                continue;
            }
            let r = minus.upper_bound(&(x[i] + t));
            if r == 0 {
                continue;
            }
            if minus[r - 1] + t < x[i] {
