...
fn solve() {
    input! {
        n:usize,a:i64,b:i64,c:i64,field:[[i64;n];n]
    }
    let mut g = Dijkstra::new(n * 2);
    g.add_edge(0, n, 0);
    for i in 0..n {
        g.add_edge(i, i + n, 0);
        for j in 0..n {
            ...
            g.add_edge(i, j, field[i][j] * a);
            g.add_edge(i + n, j + n, field[i][j] * b + c);
        }
    }
    let solve = g.calc_shortest_distance(0);
    let ans = solve[n - 1].min(solve[2 * n - 1]);
    println!("{}", ans);
}
fn main() {
    solve();
